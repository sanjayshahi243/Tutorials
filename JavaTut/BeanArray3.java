...
The array {3, 8, 4} is not a Bean array because of the value 3 which requires 
that the array contains either the value 6, 7 or 1 and none of these values are in the array.
...
public class BeanArray3 {
    static int isBean(int[] arr) {
        ...
        for (int i = 0; i < arr.length; i++) {
            ...
            for (int j = 0; j < arr.length; j++) {
                if (arr[j] == 2*arr[i] || arr[j] == 2*arr[i]+1 || arr[j] == arr[i]/2) {
                    flag = true;
                    break;
                }
            }
    ...
    public static void main(String[] args) {
        System.out.println("Result: " + isBean(new int[] {2, 2, 5, 11, 23})); // 1
        System.out.println("Result: " + isBean(new int[] {7, 7, 3, 6})); // 1
        System.out.println("Result: " + isBean(new int[] {0})); // 1
        System.out.println("Result: " + isBean(new int[] {3, 8, 4})); // 0
    }
}
